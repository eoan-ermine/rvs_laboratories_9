...
В рамках данной лабораторной работы рассматривается реализация разностной схемы с 7-точечным шаблоном на графическом процессоре с использованием технологии CUDA. Актуальность работы обусловлена тем, что современные вычислительные задачи, связанные с обработкой многомерных данных, численным моделированием и научными расчётами, требуют высокой производительности и эффективного использования вычислительных ресурсов. Одним из способов повышения скорости вычислений является перенос трудоёмких операций на GPU, обладающий высокой степенью параллелизма.

Целью лабораторной работы является освоение принципов параллельной реализации разностных схем на GPU, а также изучение возможностей разделяемой памяти для уменьшения количества обращений к глобальной памяти и повышения эффективности вычислений. В ходе выполнения работы требуется отредактировать код CUDA-программы для реализации вычисления разностной схемы с 7-точечным шаблоном, настроить запуск ядра на двумерной сетке блоков и проанализировать особенности работы алгоритма с точки зрения обращений к памяти.

Особенность рассматриваемой схемы состоит в том, что значение каждого внутреннего элемента трёхмерного массива определяется на основе шести соседних элементов по трём координатным направлениям и значения центральной точки. Полученный результат ограничивается диапазоном от 0 до 255, что соответствует дополнительному этапу нормализации выходных данных. Реализация такого алгоритма на GPU позволяет продемонстрировать преимущества параллельных вычислений, а также выявить влияние организации памяти на производительность программы.

Таким образом, выполнение лабораторной работы направлено на закрепление практических навыков разработки CUDA-ядер, использования разделяемой памяти и анализа эффективности параллельных алгоритмов при решении задач вычислительной обработки данных.

\section{Цель работы}

Получение навыков расчета разностной схемы с 7-точечным шаблоном с использованием разделяемой памяти с помощью CUDA.

\section{Задание}

Реализовать программу, выполняющую расчет разностной схемы с 7-точечным шаблоном с использованием разделяемой памяти с помощью CUDA.

\section{Ход работы}

В листинге ниже приведено содержание файла rvs\_laboratories\_9/rvs\_\-lab\-or\-at\-ories\_\-9.cu, выполняющий расчет разностной схемы с 7-точечным шаблоном с использованием разделяемой памяти на GPU с помощью CUDA (лист. \ref{lst:main}).
...
В качестве тестового изображения будет использовать фотографию, снятую в районе Зябликово г. Москвы (рис. \ref{img:input}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{images/input.png}
    \caption{Тестовое изображение\label{img:input}}
\end{figure}

На картинке ниже приведено эталонное выходное изображение (рис. \ref{img:output}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{images/output.png}
    \caption{Эталонное изображение\label{img:output}}
\end{figure}
...
На рисунке ниже приведено изображение, получившееся в результате работы программы.
...
Как видно, оно совпадает с эталонным.

\sectionc{Заключение}

Таким образом, в ходе выполнения данной лабораторной работы были успешно получены навыки расчета разностной схемы с 7-точечным шаблоном с использованием разделяемой памяти с помощью CUDA. На практике была реализована программа, выполняющая расчет разностной схемы с 7-точечным шаблоном с использованием разделяемой памяти с помощью CUDA.
